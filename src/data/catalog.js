// كل المنتجات والعروض في الموقع بتتحطّ هنا. عشان تضيفي منتج أو عرض جديد،
// انسخي أي عنصر موجود وغيّري بياناته — هيظهر على الموقع تلقائيًا من غير
// أي تعديل تاني في باقي الكود.
//
// شكل كل منتج:
// - id, name, category: بيانات أساسية.
// - price / oldPrice: السعر المعروض في كروت المنتجات والصفحة الرئيسية.
// - images: أول صورة هي الرئيسية، والباقي بيظهروا كصور مصغّرة في صفحة
//   المنتج. الصور متخزّنة في public/images/ ("/images/x.webp")، أو
//   ممكن تحطي رابط أونلاين يبدأ بـ http/https.
// - description: نص وصف عادي (من غير أي أسعار جواه).
// - options: مصفوفة اختيارات الشراء (زي "قطعة واحدة" أو "2+1 مجانًا")،
//   كل اختيار له label و qty (عدد القطع في الاختيار ده) و price و
//   oldPrice و isBundle (تتحكم في ظهور باچ "الأكثر طلبًا") و note (نص
//   إضافي زي "والشحن مجاني"). سيبيها [] لو المنتج مالوش أكتر من سعر واحد.
// - relatedOfferIds: مصفوفة id بتاعة العروض (من BUNDLES) اللي عايزة
//   تظهر تحت المنتج ده تحديدًا في قسم "عروض قد تعجبك". سيبيها [] لو مش
//   عايزة يظهر أي عرض تحت المنتج ده.
// - reviews: مصفوفة تقييمات العملاء { name, rating (من 1 لـ 5), text }.
//   سيبيها [] لحد ما يكون عندك تقييمات حقيقية تضيفيها.
// - beforeAfter: { before, after } (روابط صورتين)، أو null لو مفيش.

export const PRODUCTS = [
  {
    id: 'product-1',
    name: '🧽 فوم إزالة الدهون',
    category: ' أدوات التنظيف',
    price: 249,
    oldPrice: 310,
    description: `قوة تنظيف.. على أصعب الدهون ✨

سبراي مخصص لإزالة الدهون المتراكمة والزيوت وبقايا الطعام من أسطح المطبخ، ليساعدك على استعادة نظافة ولمعان الأسطح بسهولة وسرعة.

⭐ أهم المميزات:
✔️ يساعد على إزالة الدهون الصعبة
✔️ سريع وفعال في التنظيف
✔️ مناسب لأسطح المطبخ المختلفة
✔️ سهل الاستخدام
✔️ برائحة ليمون منعشة 🍋

🧼 طريقة الاستخدام:
رش كمية مناسبة على السطح المتسخ.
اتركه لفترة قصيرة على الدهون المتراكمة.
امسح بإسفنجة أو فوطة نظيفة.
للبقع الصعبة، كرر الاستخدام عند الحاجة.

🏠 مناسب لـ:
البوتجاز • شفاط المطبخ • أسطح المطبخ • الأحواض والأسطح المقاومة للمنظفات

📦 الحجم: 500 مل`,
    images: ['/images/product-1.webp'],
    options: [
      {
        key: '1-249',
        label: 'العبوة',
        qty: 1,
        price: 249,
        oldPrice: 310,
        isBundle: false,
        note: 'و الشحن مجانى',
      },
      {
        key: '3-499',
        label: '2 + 1 مجانا',
        qty: 3,
        price: 499,
        oldPrice: 930,
        isBundle: true,
        note: 'و الشحن مجانى سعر العبوة فى العرض 166 ج فقط',
      },
    ],
    relatedOfferIds: [],
    reviews: [],
    beforeAfter: null,
  },
  {
    id: 'product-2',
    name: '🧽 وايبس إزالة الدهون',
    category: ' أدوات التنظيف',
    price: 249,
    oldPrice: 310,
    description: `قوة تنظيف في كل مساحة ✨

وايبس مخصصة لإزالة الدهون وبقع الطعام والأوساخ الصعبة بسرعة وسهولة، لتساعدك على تنظيف المطبخ والأسطح المختلفة بدون مجهود.

⭐ أهم المميزات:
✔️ تساعد على إزالة الدهون والبقع الصعبة
✔️ سهلة وسريعة الاستخدام
✔️ مناسبة للتنظيف اليومي
✔️ تترك السطح نظيفًا ومنتعشًا
✔️ عملية وسهلة الحمل

🧼 طريقة الاستخدام:
اسحب قطعة واحدة من العبوة.
امسح بها السطح المتسخ.
للبقع الصعبة، افرك بلطف وكرر الاستخدام عند الحاجة.
لا تحتاج إلى شطف في الاستخدامات المناسبة.

🏠 مناسبة لـ:
البوتجاز • أسطح المطبخ • الرخام والأسطح الصلبة • الأجهزة والأسطح المختلفة

📦 العبوة: 80 قطعة`,
    images: ['/images/product-2.webp'],
    options: [
      {
        key: '1-249',
        label: 'العبوة',
        qty: 1,
        price: 249,
        oldPrice: 310,
        isBundle: false,
        note: 'و الشحن مجانى',
      },
      {
        key: '3-499',
        label: '2 + 1 مجانا',
        qty: 3,
        price: 499,
        oldPrice: 930,
        isBundle: true,
        note: 'و الشحن مجانى سعر العبوة فى العرض166 ج فقط',
      },
    ],
    relatedOfferIds: [],
    reviews: [],
    beforeAfter: null,
  },
  {
    id: 'product-3',
    name: '🧺 كبسولات تنظيف وتعقيم الغسالة',
    category: ' أدوات التنظيف',
    price: 229,
    oldPrice: 300,
    description: `غسالة أنظف.. ملابس أكثر صحة ✨

كبسولات مخصصة لتنظيف الغسالة من الداخل، تساعد على إزالة الأوساخ والتراكمات والروائح الكريهة، للحفاظ على نظافة الغسالة وكفاءتها.

⭐ أهم المميزات:
✔️ تنظيف عميق للغسالة
✔️ تساعد على التخلص من الروائح الكريهة
✔️ تساعد على تقليل البكتيريا والجراثيم
✔️ تساعد في الحفاظ على كفاءة الغسالة
✔️ سهلة الاستخدام ولا تحتاج مجهود

🧼 طريقة الاستخدام:
ضع كبسولة واحدة داخل حوض الغسالة الفارغ.
شغّل دورة غسيل مناسبة بدون ملابس.
اترك الدورة تكمل للنهاية.
كرر الاستخدام بشكل دوري للحفاظ على نظافة الغسالة.

🏠 مناسبة لـ:
لجميع أنواع الغسالات

📦 العبوة: 12 كبسولة`,
    images: ['/images/product-3.webp'],
    options: [
      {
        key: '1-229',
        label: 'العبوة',
        qty: 1,
        price: 229,
        oldPrice: 300,
        isBundle: false,
        note: 'و الشحن مجانى',
      },
      {
        key: '3-459',
        label: '2 + 1 مجانا',
        qty: 3,
        price: 459,
        oldPrice: 900,
        isBundle: true,
        note: 'و الشحن مجانى سعر العبوة فى العرض 153 ج فقط',
      },
    ],
    relatedOfferIds: [],
    reviews: [],
    beforeAfter: null,
  },
  {
    id: 'product-4',
    name: '🧼 أقراص تعقيم وتنظيف قاعدة الحمام',
    category: ' المطبخ',
    price: 197,
    oldPrice: 230,
    description: `نظافة أعمق.. ورائحة منعشة تدوم ✨

أقراص مخصصة لتنظيف وتعقيم قاعدة الحمام، تساعد على إزالة الأوساخ والتكلسات والروائح غير المرغوبة وتترك الحمام بمظهر أنظف ورائحة منعشة.

🚽 مناسب لـ:
قاعدة الحمام
إزالة التكلسات والبقع
التخلص من الروائح غير المرغوبة
التنظيف الدوري للحمام

⭐ أهم المميزات:
✔️ تنظيف عميق وفعّال
✔️ يساعد على إزالة البقع والتكلسات
✔️ يترك رائحة منعشة
✔️ سريع الذوبان وسهل الاستخدام
✔️ مناسب للتنظيف الدوري

🧽 طريقة الاستخدام:
ضع قرصًا واحدًا داخل قاعدة الحمام.
اتركه يذوب ويتفاعل مع الماء.
اتركه لمدة 10–30 دقيقة حسب مستوى الاتساخ.
اسحب السيفون واستمتع بقاعدة حمام أنظف.

📦 العبوة: 12 قرص`,
    images: ['/images/product-4.webp'],
    options: [
      {
        key: '1-197',
        label: 'العبوة',
        qty: 1,
        price: 197,
        oldPrice: 230,
        isBundle: false,
        note: 'و الشحن مجانى',
      },
      {
        key: '3-397',
        label: '2 + 1 مجانا',
        qty: 3,
        price: 397,
        oldPrice: 690,
        isBundle: true,
        note: 'و الشحن مجانى سعر العبوة فى العرض 133 ج فقط',
      },
    ],
    relatedOfferIds: [],
    reviews: [],
    beforeAfter: null,
  },
  {
    id: 'product-5',
    name: '🧺 كبسولات الغسيل الذكية 5 في 1',
    category: ' أدوات التنظيف',
    price: 269,
    oldPrice: 300,
    description: `نظافة أعمق.. ورائحة منعشة تدوم طويلًا ✨

كبسولات غسيل متعددة الوظائف تجمع بين التنظيف، إزالة البقع، والحفاظ على رائحة الملابس وانتعاشها، لتمنحك غسيلًا نظيفًا برائحة جميلة مع كل دورة.

⭐ أهم المميزات:
✔️ تنظيف عميق للملابس
✔️ تساعد على إزالة البقع بفعالية
✔️ تمنح الملابس رائحة منعشة تدوم طويلًا
✔️ تساعد على الحفاظ على ألوان الملابس
✔️ مناسبة لمختلف أنواع الغسيل

🧼 طريقة الاستخدام:
ضع كبسولة واحدة داخل حوض الغسالة قبل وضع الملابس.

أضف الملابس وشغّل دورة الغسيل المناسبة.

لا تحتاج لإضافة مسحوق غسيل إضافي.

استمتع بملابس نظيفة ورائحة منعشة 🌸

📦 العبوة: 50 كبسولة`,
    images: ['/images/product-5.webp'],
    options: [
      {
        key: '1-269',
        label: 'العبوة',
        qty: 1,
        price: 269,
        oldPrice: 300,
        isBundle: false,
        note: 'و الشحن مجانى',
      },
      {
        key: '3-538',
        label: '2 + 1 مجانا',
        qty: 3,
        price: 538,
        oldPrice: 900,
        isBundle: true,
        note: 'و الشحن مجانى سعر العبوة فى العرض 179 ج فقط',
      },
    ],
    relatedOfferIds: [],
    reviews: [],
    beforeAfter: null,
  },
  {
    id: 'product-6',
    name: '🧼 وايبس تنظيف البقع الصعبة من الملابس',
    category: ' أدوات التنظيف',
    price: 197,
    oldPrice: 230,
    description: `ة ✨

وايبس مخصصة للمساعدة في تنظيف البقع والأوساخ الصعبة من الملابس بسرعة وسهولة، بدون الحاجة لغسل القطعة بالكامل في كل مرة.

⭐ أهم المميزات:
✔️ تساعد على إزالة البقع الصعبة
✔️ مناسبة للاستخدام السريع
✔️ سهلة الحمل والاستخدام
✔️ تساعد على الحفاظ على ألوان الملابس
✔️ مناسبة لمختلف أنواع الأقمشة

🧽 طريقة الاستخدام:
امسح البقعة أولًا بقطعة الوايبس.
افرك البقعة بلطف حتى تبدأ في الاختفاء.
كرر الاستخدام عند الحاجة.
اترك الملابس لتجف أو اغسلها بالطريقة المعتادة.

📦 العبوة: 80 قطعة`,
    images: ['/images/product-6.webp'],
    options: [
      {
        key: '1-197',
        label: 'العبوة',
        qty: 1,
        price: 197,
        oldPrice: 230,
        isBundle: false,
        note: 'و الشحن مجانى',
      },
      {
        key: '3-399',
        label: '2 + 1 مجانا',
        qty: 3,
        price: 399,
        oldPrice: 690,
        isBundle: true,
        note: 'و الشحن مجانى سعر العبوة فى العرض 133 ج فقط',
      },
    ],
    relatedOfferIds: [],
    reviews: [],
    beforeAfter: null,
  },
  {
    id: 'product-7',
    name: '👟 وايبس تنظيف وتلميع الأحذية',
    category: ' أدوات التنظيف',
    price: 197,
    oldPrice: 230,
    description: `نظافة فورية.. في أي وقت وأي مكان ✨

وايبس مخصصة لتنظيف الأحذية وإزالة الأوساخ والبقع وآثار الاستخدام اليومية بسهولة وسرعة، لتحافظ على مظهر حذائك نظيفًا وجديدًا.

⭐ أهم المميزات:
✔️ تساعد على إزالة البقع والأوساخ
✔️ تنظيف سريع بدون ماء
✔️ سهلة الاستخدام والحمل
✔️ مناسبة للاستخدام اليومي والسفر
✔️ مناسبة لمختلف أنواع الأحذية

🧼 طريقة الاستخدام:
افتح العبوة واسحب قطعة واحدة.
امسح الحذاء بلطف على المناطق المتسخة.
كرر على البقع الصعبة عند الحاجة.
اترك الحذاء ليجف.

📦 العبوة: 80 قطعة`,
    images: ['/images/product-7.webp'],
    options: [
      {
        key: '1-197',
        label: 'العبوة',
        qty: 1,
        price: 197,
        oldPrice: 230,
        isBundle: false,
        note: 'و الشحن مجانى',
      },
      {
        key: '3-399',
        label: '2 + 1 مجانا',
        qty: 3,
        price: 399,
        oldPrice: 690,
        isBundle: true,
        note: 'و الشحن مجانى سعر العبوة فى العرض 133 ج فقط',
      },
    ],
    relatedOfferIds: [],
    reviews: [],
    beforeAfter: null,
  },
  {
    id: 'product-8',
    name: '🧽 فوطة التنظيف متعددة الاستخدامات',
    category: 'أدوات التنظيف',
    price: 237,
    oldPrice: 300,
    description: `تنظيف أسهل .. لكل مكان في بيتك ✨

فوطة عملية تساعدك على تنظيف وتجفيف الأسطح المختلفة بسهولة، وتمتاز بقدرة جيدة على امتصاص المياه والأوساخ مع إمكانية استخدامها أكثر من مرة.

🏠 مناسبة لـ:
🍳 المطبخ والأسطح
🪟 الزجاج والمرايات
🪑 الأثاث والأسطح المختلفة
🚗 تنظيف السيارة
🧼 الاستخدام اليومي في المنزل

⭐ أهم المميزات:
✔️ امتصاص قوي للمياه
✔️ تساعد على إزالة الأوساخ بسهولة
✔️ قابلة لإعادة الاستخدام
✔️ لا تترك وبرًا بشكل مزعج
✔️ مناسبة للاستخدامات المتعددة

🧼 طريقة الاستخدام:
استخدمها جافة للتنظيف وإزالة الأتربة.
بللها بالماء عند الحاجة للتنظيف الرطب.
امسح السطح جيدًا.
بعد الاستخدام تُغسل وتُترك لتجف، ويمكن إعادة استخدامها.

📦 العبوة: 20 فوطة`,
    images: ['/images/product-8.webp'],
    options: [
      {
        key: '1-237',
        label: 'العبوة',
        qty: 1,
        price: 237,
        oldPrice: 300,
        isBundle: false,
        note: 'و الشحن مجانى',
      },
      {
        key: '3-473',
        label: '2 + 1 مجانا',
        qty: 3,
        price: 473,
        oldPrice: 900,
        isBundle: true,
        note: 'و الشحن مجانى  سعر العبوة فى العرض 155 ج فقط⸻',
      },
    ],
    relatedOfferIds: [],
    reviews: [],
    beforeAfter: null,
  },
  {
    id: 'product-9',
    name: '🚽 حقن تعقيم و تعطير التواليت ',
    category: ' المطبخ',
    price: 177,
    oldPrice: 230,
    description: `تعقيم وانتعاش يدوم.. مع كل استخدام ✨

جل مخصص لقاعدة الحمام يساعد على التعقيم، تقليل الروائح غير المرغوبة وترك رائحة منعشة، مع تصميم سهل الاستخدام يمنح حمامك إحساسًا بالنظافة والانتعاش.

⭐ أهم المميزات:
✔️ يساعد على تعقيم قاعدة الحمام
✔️ يترك رائحة منعشة تدوم
✔️ يساعد على تقليل الروائح غير المرغوبة
✔️ سهل وسريع الاستخدام
✔️ يمنح الحمام انتعاشًا مستمرًا

🧼 طريقة الاستخدام:
قم بوضع السرنجة ثم قم بالضغط عليها داخل قاعدة الحمام ل تطبع اشكال ورود مميزة .
اتركها تعمل مع تدفق المياه.
مع كل سحب للسيفون تنتشر الرائحة ويستمر تأثير المنتج.
استبدل العبوة عند انتهاء الجل.

📦 العبوة: تكفي حتى 14 استخدام`,
    images: ['/images/product-9.webp'],
    options: [
      {
        key: '1-177',
        label: 'العبوة',
        qty: 1,
        price: 177,
        oldPrice: 230,
        isBundle: false,
        note: 'و الشحن مجانى',
      },
      {
        key: '3-357',
        label: '2 + 1 مجانا',
        qty: 3,
        price: 357,
        oldPrice: 690,
        isBundle: true,
        note: 'و الشحن مجانى سعر العبوة فى العرض 119 ج فقط',
      },
    ],
    relatedOfferIds: [],
    reviews: [],
    beforeAfter: null,
  },
  {
    id: 'product-10',
    name: '✨ معجون إزالة البرومة والصدأ 🛠️',
    category: ' أدوات التنظيف',
    price: 223,
    oldPrice: 290,
    description: `💥 ودّع الصدأ والبرومة والبهتان!

معجون تنظيف وتلميع قوي يساعدك على استعادة شكل المعادن والأسطح التي فقدت لمعانها بسبب الصدأ والبرومة وآثار الاستخدام.

استخدمه على:

🔩 المعادن والأجزاء المعدنية
يساعد على إزالة الصدأ والبرومة وآثار الأكسدة.

🚗 أجزاء السيارة المعدنية
لتنظيف وتلميع الأجزاء المعدنية وإظهارها بشكل أنظف.

🚿 الحنفيات والإكسسوارات المعدنية
يساعد على إزالة البهتان والبقع المتراكمة واستعادة اللمعان.

🍳 الستانلس وأدوات المطبخ
لإزالة البقع وآثار الاستخدام وتلميع السطح.

🛠️ المعادن القديمة والباهتة
يساعد على تحسين المظهر وإعادة اللمعة للسطح.

✨ النتيجة؟

تنظيف + إزالة الصدأ والبرومة + تلميع = معدن شكله أنظف وأجدد

الحجم: 500 جم`,
    images: ['/images/product-10.webp'],
    options: [
      {
        key: '1-223',
        label: 'العبوة',
        qty: 1,
        price: 223,
        oldPrice: 290,
        isBundle: false,
        note: 'و الشحن مجانى',
      },
      {
        key: '1-447',
        label: '3 عبوات',
        qty: 1,
        price: 447,
        oldPrice: 870,
        isBundle: false,
        note: 'و الشحن مجانى سعر العبوة فى العرض 179 ج فقط',
      },
    ],
    relatedOfferIds: ['offer-1', 'offer-2', 'offer-3'],
    reviews: [],
    beforeAfter: '/images/before-after/p-10.webp',
  },
  {
    id: 'product-11',
    name: '🧼 فوم تلميع وتنظيف الزجاج',
    category: ' أدوات التنظيف',
    price: 249,
    oldPrice: 310,
    description: `رؤية أوضح.. ولمعان يدوم ✨

فوم مخصص لتنظيف الزجاج وإزالة الزيوت، الأوساخ والأوساخ المتراكمة، ليساعد على استعادة صفاء ولمعان الزجاج بسهولة.

⭐ أهم المميزات:
✔️ يساعد على إزالة طبقة الزيوت والأوساخ
✔️ يعيد للزجاج مظهره اللامع
✔️ سهل وسريع الاستخدام
✔️ مناسب لمختلف الأسطح الزجاجية
✔️ يساعد على الحصول على رؤية أوضح

🧽 طريقة الاستخدام:
رش كمية مناسبة على سطح الزجاج.
افرك بلطف باستخدام إسفنجة أو فوطة ناعمة.
امسح بقطعة مايكروفايبر نظيفة.
كرر الاستخدام عند الحاجة للحصول على أفضل نتيجة.

🏠🚗 مناسب لـ:
زجاج السيارات • نوافذ المنزل • المرايات • الأسطح الزجاجية`,
    images: ['/images/product-11.webp'],
    options: [
      {
        key: '1-249',
        label: 'العبوة',
        qty: 1,
        price: 249,
        oldPrice: 310,
        isBundle: false,
        note: 'و الشحن مجانى',
      },
      {
        key: '3-499',
        label: '2 + 1 مجانا',
        qty: 3,
        price: 499,
        oldPrice: 930,
        isBundle: true,
        note: 'و الشحن مجانى سعر العبوة فى العرض 166 ج فقط',
      },
    ],
    relatedOfferIds: [],
    reviews: [],
    beforeAfter: null,
  },
  {
    id: 'product-12',
    name: '🧽 إسفنجة الميلامين لإزالة الأوساخ والبقع',
    category: 'أدوات التنظيف',
    price: 237,
    oldPrice: 300,
    description: `تنظيف عميق.. بدون منظفات ✨

إسفنجة الميلامين تساعد على إزالة العلامات، البقع والأوساخ الصعبة من الأسطح المختلفة باستخدام الماء فقط، لتساعدك على استعادة نظافة السطح بسهولة.

🏠 مناسبة لـ:
الحوائط والبقع والعلامات
البلاط والفواصل
الأحواض والأسطح الصلبة
الأثاث والأسطح القابلة للتنظيف
بعض الأحذية والملابس لإزالة العلامات السطحية

⭐ أهم المميزات:
✔️ تستخدم بالماء فقط
✔️ تساعد على إزالة البقع والعلامات الصعبة
✔️ سهلة وسريعة الاستخدام
✔️ لا تحتاج لإضافة منظفات
✔️ مناسبة لاستخدامات متعددة في المنزل

🧼 طريقة الاستخدام:
بلل الإسفنجة بالماء واعصرها جيدًا.
افرك البقعة أو العلامة بلطف.
امسح بقطعة فوطة نظيفة.
كرر عند الحاجة.`,
    images: ['/images/product-12.webp'],
    options: [
      {
        key: '4-237',
        label: 'عرض خاص عدد 4',
        qty: 4,
        price: 237,
        oldPrice: 300,
        isBundle: true,
        note: 'و الشحن مجانى',
      },
    ],
    relatedOfferIds: [],
    reviews: [],
    beforeAfter: null,
  },
  {
    id: 'product-13',
    name: '🍽️ أكياس تغطية الأطباق وحفظ الطعام',
    category: 'أدوات تنظيم',
    price: 297,
    oldPrice: 400,
    description: `حافظ على طعامك طازجًا.. ونظّم مطبخك بسهولة ✨

أكياس عملية لتغطية الأطباق وحفظ الطعام، تساعد على حماية الأكل من الأتربة والحشرات والعوامل الخارجية، وتحافظ على ترتيب ونظافة المطبخ.

⭐ أهم المميزات:
✔️ مناسبة لتغطية الأطباق والأوعية
✔️ تساعد على الحفاظ على الطعام طازجًا
✔️ سهلة الاستخدام والإغلاق
✔️ قابلة لإعادة الاستخدام
✔️ تهوية مناسبة للطعام حسب الاستخدام

🥗 مناسبة لـ:
الفواكه • الخضروات • الأطباق • المخبوزات • البطاطس • البصل والثوم • مختلف أنواع الطعام

🧺 طريقة الاستخدام:
ضع الطبق أو الطعام داخل الكيس.
اسحب الرباط لإغلاق الكيس بإحكام.
احفظ الطعام في المكان المناسب حسب نوعه.
يمكن تنظيف الكيس وإعادة استخدامه.

📦 العبوة: 100 كيس`,
    images: ['/images/product-13.webp'],
    options: [
      {
        key: '1-297',
        label: 'ال 400 كيس',
        qty: 1,
        price: 297,
        oldPrice: 400,
        isBundle: false,
        note: 'و الشحن مجانى',
      },
    ],
    relatedOfferIds: [],
    reviews: [],
    beforeAfter: null,
  },
  {
    id: 'product-14',
    name: '👜 شنطة سفر كورية',
    category: ' أدوات التنظيف',
    price: 367,
    oldPrice: 450,
    description: `رفيقتك في كل مكان ✈️
  
  شنطة سفر عملية وأنيقة، مصممة لتوفر لك مساحة واسعة وتنظيم أفضل أثناء السفر والرحلات، مع سهولة حملها واستخدامها في مختلف المناسبات.
  
  ⭐ أهم المميزات:
  ✔️ مساحة داخلية واسعة
  ✔️ جيوب داخلية لتنظيم الأغراض
  ✔️ حزام جانبي قابل للتعديل
  ✔️ سوستة متينة وعالية الجودة
  ✔️ خامة مقاومة للماء
  ✔️ تصميم عملي وأنيق
  
  🧳 مناسبة لـ:
  السفر • الرحلات • الجيم والرياضة • الاستخدام اليومي
  
  📦 تصميم عملي:
  تساعدك على ترتيب ملابسك وأغراضك الشخصية بسهولة، مع إمكانية حملها بشكل مريح أثناء التنقل.
  
  شنطة واحدة.. لكل مشاويرك ❤️`,
    images: ['/images/product-14.webp'],
    options: [
      {
        key: '1-367',
        label: 'الشنطة',
        qty: 1,
        price: 367,
        oldPrice: 450,
        isBundle: false,
        note: 'والشحن مجانى',
      },
      {
        key: '3-733',
        label: '2 + 1 مجانا',
        qty: 3,
        price: 733,
        oldPrice: 1350,
        isBundle: true,
        note: 'و الشحن مجانى',
      },
    ],
    relatedOfferIds: [],
    reviews: [],
    beforeAfter: null,
  },
  {
    id: 'product-15',
    name: '🎨 مجموعة الإبداع – ألوان تاتش 60 قطعة مع كتاب التلوين الجاهز',
    category: ' أدوات التنظيف',
    price: 529,
    oldPrice: 650,
    description: `ألوان أكتر.. متعة أكبر ✨

مجموعة متكاملة للرسم والتلوين، تضم 60 لون تاتش زاهي وواضح مع كتب تلوين جاهزة، لتستمتع بالتلوين وتطلق خيالك وتحوّل كل رسمة لعمل مميز.

⭐ أهم المميزات:
✔️ 60 لونًا متنوعًا وزاهيًا
✔️ رأسين للاستخدام المتعدد
✔️ كتب تلوين برسومات جاهزة
✔️ ألوان واضحة وسهلة الاستخدام
✔️ تساعد على تنمية الإبداع والخيال
✔️ مناسبة للأطفال والكبار

🖍️ محتويات المجموعة:
60 لون تاتش + كتاب تلوين جاهزة 🎨

🎁 مناسبة لـ:
الأطفال • محبي الرسم والتلوين • الأنشطة الإبداعية • الهدايا

لون حياتك.. بكل تفاصيلها ❤️`,
    images: ['/images/product-15.webp'],
    options: [
      {
        key: '1-529',
        label: 'المجموعة',
        qty: 1,
        price: 529,
        oldPrice: 650,
        isBundle: false,
        note: 'و الشحن مجانى',
      },
      {
        key: '3-1059',
        label: 'اشترى2 + 1 مجانا  و الشحن مجانى',
        qty: 3,
        price: 1059,
        oldPrice: 1950,
        isBundle: true,
        note: 'و الشحن مجانى - المجموعة سعرها فى العرض 353 ج فقط',
      },
    ],
    relatedOfferIds: [],
    reviews: [],
    beforeAfter: null,
  },
];

export const BUNDLES = [
  {
    id: 'offer-1',
    name: '🧼 باكدج المطبخ النظيف',
    category: 'عرض',
    price: 437,
    oldPrice: null,
    description: `كل ما يحتاجه مطبخك.. في عرض واحد ✨

باكدج متكامل يجمع لك 4 منتجات أساسية لتنظيف وترتيب المطبخ، عشان تخلص من الدهون والأوساخ وتحافظ على مطبخك نظيف ومرتب بسهولة.

📦 محتويات العرض:
🧽 وايبس إزالة الدهون – 80 قطعة
تساعد على إزالة الدهون وبقع الطعام بسرعة وسهولة.

✨ سبراي إزالة الدهون
للتعامل مع الدهون والتراكمات الصعبة على أسطح المطبخ.

🧺 فوطة تنظيف متعددة الاستخدامات – 20 قطعة
لامتصاص المياه وتنظيف وتجفيف الأسطح المختلفة.

🍽️ أكياس تغطية الأطباق وحفظ الطعام – 100 كيس
لتغطية الأطباق وحماية الطعام من الأتربة والحشرات وتنظيم المطبخ.

⭐ ليه العرض ده؟
✔️ 4 منتجات أساسية في باكدج واحد
✔️ تنظيف أسهل وأسرع
✔️ مناسب للاستخدام اليومي
✔️ يوفر عليك شراء كل منتج بشكل منفصل

🔥 سعر العرض:
437 جنيه فقط

🚚 الشحن مجانًا
💳 الدفع عند الاستلام
🛡️ منتجات أصلية`,
    images: ['/images/offer-1.webp'],
    includes: ['product-2', 'product-1', 'product-8', 'product-13'],
    relatedOfferIds: ['offer-2', 'offer-3'],
    reviews: [],
    beforeAfter: null,
  },
  {
    id: 'offer-2',
    name: '🚿 عرض الحمام النظيف',
    category: 'عرض',
    price: 449,
    oldPrice: null,
    description: `كل ما يحتاجه حمامك.. في عرض واحد ✨

باكدج متكامل للعناية بنظافة الحمام وانتعاشه، يجمع 4 منتجات أساسية تساعدك على الحفاظ على حمام أنظف، أكثر انتعاشًا وأسهل في التنظيف.

📦 محتويات العرض:
🧼 أقراص تنظيف قاعدة الحمام – 12 قرص
تساعد على تنظيف قاعدة الحمام وتقليل الروائح غير المرغوبة.

🚽 جل تعقيم وتعطير قاعدة الحمام – حتى 14 استخدام
يساعد على التعقيم ويمنح الحمام رائحة منعشة مع الاستخدام.

🧺 كبسولات تنظيف وتعقيم الغسالة – 12 كبسولة
لتنظيف الغسالة من الداخل والمساعدة على التخلص من التراكمات والروائح.

🌸 كبسولات الغسيل 5 في 1 – 50 كبسولة
للتنظيف وإزالة البقع ومنح الملابس رائحة منعشة.

⭐ ليه تختار العرض؟
✔️ 4 منتجات في باكدج واحد
✔️ نظافة الحمام والملابس والغسالة في عرض واحد
✔️ عملي للاستخدام اليومي
✔️ توفير بدل شراء المنتجات بشكل منفصل

🔥 سعر العرض:
449 جنيه فقط

🚚 الشحن مجانًا
💳 الدفع عند الاستلام`,
    images: ['/images/offer-2.webp'],
    includes: ['product-4', 'product-9', 'product-3', 'product-5'],
    relatedOfferIds: ['offer-1', 'offer-3'],
    reviews: [],
    beforeAfter: null,
  },
  {
    id: 'offer-3',
    name: '🏠 عرض البيت المتكامل النظيف',
    category: 'عرض',
    price: 879,
    oldPrice: null,
    description: `كل اللي يحتاجه بيتك.. في عرض واحد ✨

باكدج متكامل يجمع مجموعة من منتجات التنظيف والعناية بالمنزل، لتسهيل التنظيف اليومي والتعامل مع الدهون والبقع والروائح في مختلف أنحاء البيت.

📦 محتويات العرض:
🧽 وايبس إزالة الدهون
✨ سبراي إزالة الدهون
🧼 معجون تنظيف وتلميع الاستانلس
🚽 أقراص تنظيف وتعقيم قاعدة الحمام
🧴 جل تعقيم وتعطير قاعدة الحمام
🧺 كبسولات تنظيف وتعقيم الغسالة
🌸 كبسولات الغسيل 5 في 1
👟 وايبس تنظيف الأحذية
👕 وايبس تنظيف البقع الصعبة من الملابس
🧽 إسفنجة الميلامين لإزالة البقع والأوساخ
🧹 فوط تنظيف متعددة الاستخدامات
🍽️ أكياس تغطية الأطباق وحفظ الطعام

⭐ مميزات العرض:
✔️ مجموعة متكاملة لاحتياجات البيت
✔️ منتجات متعددة لاستخدامات مختلفة
✔️ مناسبة للتنظيف اليومي
✔️ توفر عليك شراء المنتجات بشكل منفصل

🔥 سعر العرض:
879 جنيه فقط

🚚 الشحن مجانًا
💳 الدفع عند الاستلام متوفر

بيت أنظف.. وحياة أسهل ❤️`,
    images: ['/images/offer-3.webp'],
    includes: [
      'product-2',
      'product-1',
      'product-10',
      'product-4',
      'product-9',
      'product-3',
      'product-5',
      'product-7',
      'product-6',
      'product-12',
      'product-8',
      'product-13',
    ],
    relatedOfferIds: ['offer-1', 'offer-2'],
    reviews: [],
    beforeAfter: null,
  },
];
